...
the active kit (kit-u1: Playwright TypeScript) determines the technology stack for automation
artifacts. Always read `kit.config.json` first before any automation task.
...
2. Read `tc-to-automate/kits/kit-u1/KIT.md` for kit-specific generation rules and naming conventions
3. Check `tc-to-automate/configs/integrations.yml` for active plugins (jira, github, allureServer)
4. Confirm active kit to the user in one line: `Active kit: kit-u1 | playwright | <testStyle>`

> **Note**: Kit context (Steps 1–4) is required for Capability 3 automation commands only.
> For `/gen-user-stories` and `/gen-manual-tests`, no kit context is needed — those commands
> are kit-independent and resolve their settings from command arguments.
...
**Commands**: `/gen-user-stories` (stories only or stories + manual TCs) · `/e2e --req` (full pipeline)
...
**Commands**: `/gen-manual-tests` (manual TCs only) · `/e2e --story` (manual TCs + automation)
...
| `/e2e --story` | [same as above] → **intake-agent** → locator-agent → pom-agent → testgen-agent or bdd-agent → reporting-agent |
...
`/gen-manual-tests` and `/e2e --story` as a `--story` input.
...
It is kit-aware — kit-u1 (Playwright TypeScript) is the active kit.

```
tc-to-automate/
├── kits/
│   ├── _base/                   ← shared agents + skills
│   │   ├── agents/              ← intake, locator, pom, testgen, bdd, datagen, ci, reporting
│   │   └── skills/              ← selector-scorer, gherkin-transformer, data-factory, etc.
│   └── kit-u1/                  ← Playwright TypeScript (ACTIVE KIT)
│       ├── KIT.md               ← kit-specific generation rules + naming conventions
│       ├── agents/              ← framework-setup-agent
│       └── templates/           ← Handlebars templates (9 total)
├── configs/
...
**Kit**: Configured via `kit.config.json` at project root.
...
4. **Validate after generation.** After any code generation, run `tsc --noEmit` to confirm no
   compile errors. Validation is built into `/setup-kit` and runs automatically.
...
The active kit is kit-u1 (Playwright TypeScript). The `kit.config.json` at root controls all
kit settings. To extend with a new kit in the future: create `tc-to-automate/kits/<new-id>/`
with a `KIT.md`, `kit.config.json` (extending `_base`), and any kit-specific templates.

The `story-to-testcase/` and `req-to-story/` modules are kit-independent and shared across all
kits. They require no changes when kit configuration is updated.
