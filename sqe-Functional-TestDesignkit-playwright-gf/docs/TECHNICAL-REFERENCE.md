...
**Version**: 2.1 | **Kit ID**: `kit-u1` | **Date**: 2026-03-16
...
2. Read `tc-to-automate/kits/kit-u1/KIT.md` — kit-specific naming conventions and generation rules
3. Read `tc-to-automate/configs/integrations.yml` — check active plugins (jira, github, allureServer)
4. Confirm active kit: `Active kit: kit-u1 | playwright | <testStyle>`

> Kit context (Steps 1–4) is required for Capability 3 automation commands only.
> `/gen-user-stories` and `/gen-manual-tests` are kit-independent — no kit context needed.
...
| `/e2e --story` | [same as above] → intake-agent → locator-agent → pom-agent → testgen/bdd-agent → reporting-agent |
| `/e2e --req` | req-ingestor → req-analyzer → feature-splitter → user-story-writer → [for each US: same as `/e2e --story`] |
...
**Input Type 2 — Test Cases → Automation:**

| Slash Command | Agents Spawned (in order) |
|---|---|
| `/setup-kit` | scaffolder-agent → validation (tsc + playwright list + cucumber dry-run) |
...
4. Validate after generation: `npx tsc --noEmit` — built into `/setup-kit` automatically
...
This kit provides 8 commands.

### `/gen-user-stories`
**File**: `.claude/commands/gen-user-stories.md`
**Purpose**: Input Type 0 entry point — extract user stories from requirement documents (BRD, PRD, Feature Spec, meeting notes, images, etc.). Kit-independent — no kit context needed.
...
**Purpose**: User story → manual test case documents (BDD `.feature` or non-BDD `.md`). Does NOT touch the automation pipeline. Kit-independent — no kit context needed.

| Argument | Description |
|---|---|
| `--story <path\|inline>` | User story file (`.md`, `.txt`, `.pdf`, `.yml`) or inline text |
| `--style bdd\|nonbdd\|both` | Output format (default: `both`) |
...
**Purpose**: Full end-to-end pipeline. Accepts either a **requirement document** (`--req`) or a **user story** (`--story`). Generates manual TCs then automation artifacts. `--req` and `--story` are mutually exclusive.

| Argument | Description |
|---|---|
| `--req <path>` | Requirement document — runs req-to-story pipeline first, then processes each US |
| `--story <path\|inline>` | User story file or inline text (skips req-to-story) |
...
| `--type <doctype>` | Doc type hint (only when `--req` is used) |
| `--limit <n>` | Max stories from req doc (only when `--req` is used) |
| `--dry-run` | Preview without writing |

**Pipeline (--req)**: req-ingestor → req-analyzer → feature-splitter → user-story-writer → [for each US: user-story-parser → quality-master-orchestrator → tc-formatter → intake-agent → locator-agent → pom-agent → testgen/bdd-agent → reporting-agent] → validate → summary

**Pipeline (--story)**: user-story-parser → quality-master-orchestrator → tc-formatter → [intake-agent → locator-agent → pom-agent → testgen/bdd-agent → reporting-agent] → validate → summary
...
**Purpose**: Scaffold the Playwright TypeScript project (installs dependencies, creates directory structure, generates `playwright.config.ts`). Validation is built in — runs `tsc --noEmit`, `playwright test --list`, and BDD dry-run automatically after scaffold.

> Validation is part of `/setup-kit` — no separate validate command needed.
...
This kit (kit-u1: Playwright TypeScript) uses three rule files.
...
- TypeScript actions return `Promise<void>`
...
> To run the full pipeline (req → stories → manual TCs → automation), use `/e2e --req` instead.
...
```json
{
  ...
  "strategyDefaults": { ... },
  "dependencies": { ... },
  "projectName": "sqe-Functional-TestDesignkit-playwright-gf"
}
```
